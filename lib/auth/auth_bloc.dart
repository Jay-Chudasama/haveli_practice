...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  AuthBloc(this.run) : super(Init()) {
    on<Authenticate>((event, emit) async {
      ...
      try{
        var response = await repo.loadUserDetailsApi();
        emit(Authenticated(Userdetails.fromJson(response.data)));
      }catch(value){
  ...
  void onChange(Change<AuthState> change) {
    ...
    if(change.nextState is Authenticated || change.nextState is Failed || change.nextState is UnAuthenticated){
      run();
    }
  }
}
